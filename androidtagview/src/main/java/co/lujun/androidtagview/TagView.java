...
public class TagView extends View {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    int width =
        mHorizontalPadding * 2 + (int) fontW + (isEnableCross() ? height : 0) + (isEnableImage()
            ? height : 0);
  ...
  private void drawImage(Canvas canvas) {
    if (isEnableImage()) {
      Bitmap scaledImageBitmap =
          Bitmap.createScaledBitmap(mBitmapImage, Math.round(getHeight() - mBorderWidth),
              Math.round(getHeight() - mBorderWidth), false);

      Paint paint = new Paint();
      paint.setAntiAlias(true);
      paint.setShader(
          new BitmapShader(scaledImageBitmap, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP));
      RectF rect = new RectF(mBorderWidth, mBorderWidth, getHeight() - mBorderWidth,
  ...
  private void drawCross(Canvas canvas) {
    if (isEnableCross()) {
      mPaint.setStyle(Paint.Style.FILL);
      mPaint.setColor(mCrossBgColor);
      if (mTextDirection == View.TEXT_DIRECTION_RTL) {
        RectF rect = new RectF(mBorderWidth, mBorderWidth, getHeight() - mBorderWidth,
            getHeight() - mBorderWidth);
        canvas.drawRoundRect(rect, mBorderRadius, mBorderRadius, mPaint);
      } else {
        RectF rect = new RectF(getWidth() - getHeight() - mBorderWidth, mBorderWidth,
  ...
  public int getCrossBgColor() {
    return mCrossBgColor;
  }

  public void setCrossBgColor(int mCrossBgColor) {
    this.mCrossBgColor = mCrossBgColor;
  }
